...
import com.github.bluecatlee.ccb.bean.request.H5PayRequest;
...
//        Assert.hasLength(publicKey, "建行支付公钥配置不能为空");
     ...
     * @return
     */
    public <T extends CCBResponseBody> CCBBaseResponse<T> executeRequestNew(CCBRequestBody body, Class<T> clazz) {
        return this.executeRequestNew(body, clazz, true);
    }
     ...
     * @return
     */
    public <T extends CCBResponseBody> CCBBaseResponse<T> executeRequestNew(CCBRequestBody body, Class<T> clazz, boolean connect) {
...
//                result = new String(resp.getBytes(CHARSET_UTF_8), CHARSET_GB18030);       // 这里有坑 这种方式编码转换之后依旧是乱码 应该直接获取字节数据再编码
                ...
                // 直接解析
                JavaType parametricType = XML_MAPPER.getTypeFactory().constructFromCanonical(clazz.getCanonicalName());
                JavaType javaType = XML_MAPPER.getTypeFactory().constructParametricType(CCBBaseResponse.class, parametricType);
                CCBBaseResponse<T> ccbBaseResponse = XML_MAPPER.readValue(result, javaType);
...
//                result = URLDecoder.decode(resp, CHARSET_GB18030);
    ...
    public String executeRequest(CCBRequestBody body, boolean connect) {
        ...
        // 先执行连接请求 todo 建行要求每次请求外联平台客户端前都要连接一次 个人感觉没必要
        if (connect) {
            this.connect();
        }
...
//        }, false);
        CCBBaseResponse<ConnectResponse> baseResponse = this.executeRequestNew(connectRequest, ConnectResponse.class, false);
...
//        request.setReferer("");
    ...
    public void prepare(H5PayRequest request) {
        // 设置基础参数
        request.setMerchantId(custId);
        request.setPosId(posId);
        request.setBranchId(branchId);
    ...
    public String prePay(H5PayRequest request) throws IllegalAccessException {
        // 设置基础参数
        request.setMerchantId(custId);
        request.setPosId(posId);
        request.setBranchId(branchId);
    ...
    public String calMac(H5PayRequest request) {
        Map<String, String> params = new LinkedHashMap<>();
        params.put("MERCHANTID", request.getMerchantId());
        params.put("POSID", request.getPosId());
        params.put("BRANCHID", request.getBranchId());
        ...
        params.put("CURCODE", request.getCurCode());
        params.put("TXCODE", request.getTxCode());
        params.put("REMARK1", request.getRemark1() == null ? "" : request.getRemark1());
        params.put("REMARK2", request.getRemark2() == null ? "" : request.getRemark2());
        params.put("TYPE", request.getType());
        params.put("PUB", request.getPub());
        params.put("GATEWAY", request.getGateway());
        if (request.getClientIp() != null) {
        ...
        if (StringUtil.isNotNullOrBlankTrim(request.getLzfIdType())) {
            params.put("LZFIDTYPE", request.getLzfIdType());
        }
        if (StringUtil.isNotNullOrBlankTrim(request.getLzfUserId())) {
            params.put("LZFUSERID", request.getLzfUserId());
        }
        if (StringUtil.isNotNullOrBlankTrim(request.getLzfUsername())) {
            params.put("LZFUSERNAME", request.getLzfUsername());
        }
        if (StringUtil.isNotNullOrBlankTrim(request.getLzfMobile())) {
            params.put("LZFMOBILE", request.getLzfMobile());
        }

        if (StringUtil.isNotNullOrBlankTrim(request.getMerchantUrl())) {
            params.put("Mrch_url", request.getMerchantUrl());
        }
