 ...
 */
public class CCBEbsPayRequestClient extends CCBPayBaseRequestClient {

    /**
     * 外联平台支付流水查询（5w1002）
     * @author   v1.0   2021/10/7
     */
    ...
    public File downLoadReconciliationFile(CCBEbsPayQueryFileReq model, String ip, String port, Map<String, String> ftpConfig, String filePath) throws CCBPayApiException {
        ...
        if (StringUtils.isBlank(fileName)) {
            logger.info("调用 5W1005 接口生成所需要的回单文件，文件名为空");
            return null;
        }
        ...
        try {
            ...
            if (!FTPUtils.connect(ftpClient, ftpIp, Integer.valueOf(ftpPort), ftpUserName, ftpPassWord)) {
                logger.error(String.format("地址[%s] 端口[%s] 用户名[%s] 密码[%s] 登陆失败", ftpIp,
                        Integer.valueOf(ftpPort), ftpUserName, ftpPassWord));
                return null;
            }
            if (!FTPUtils.download(ftpClient, "/", fileName, filePath)) {
                logger.info("从外联平台下载回单文件失败");
                return null;
            }
        } catch (Exception e) {
            throw new CCBPayApiException(e);
